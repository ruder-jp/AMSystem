...
#define WORKS_SQL_CREATE @"CREATE TABLE IF NOT EXISTS works (id INTEGER PRIMARY KEY AUTOINCREMENT, date REAL,start INTEGER ,end INTEGER,time_id INTEGER REFERENCES times(id),rest_id INTEGER REFERENCES rests(id));"
...
- (BOOL)updateEnd:(Work *)work
{
    ...
    NSLog(@"%i",count);
    BOOL isSucceeded = [db executeUpdate:SQL_END,  work.end,[NSNumber numberWithInteger:count]];
	[db close];
    NSLog(@"%@",@"update");
...
-(NSArray*)datas
{
    ...
    while([results next]){
        ...
        work.start = [results dateForColumnIndex:2];
        work.end = [results dateForColumnIndex:3];
